...
def talker_position_stamped():
    ...
    pub_mode = rospy.Publisher('/mode', std_msgs.msg.String, queue_size=1)
    rospy.init_node('talker', anonymous=True)
    rate = rospy.Rate(1) # 1hz

    seq = [[0.0, 0.5, 0.00], [0.7, 0, 0.5], [0.07, -0.3, 0.25], [-2, -0.2, 0.5], [0.7, 0, 0.5]] #,-0.05, -0.1, -0.15, -0.1, -0.05, 0]
    #seq = [[0.1, 0.7, 0]] #,-0.05, -0.1, -0.15, -0.1, -0.05, 0] [0.1, -0.8, 0.00]] #,
    seq_count = 0
    round_count = 0
    pub_mode.publish('look_at_object')
    while not rospy.is_shutdown():
    ...
    pub_mode.publish('follow_person')
